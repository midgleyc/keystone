...
export async function validateCustomAccessControl<Args>(
  args: { access: CustomAccess<Args> } & Args
) {
...
>(args: { access: ListAccess<Args> } & Args) {
  // Either a boolean or an object describing a where clause
  let result: Static | Declarative = false;
  const acc = args.access[args.operation];
...
>(args: { access: FieldAccess<Args> } & Args) {
...
export async function validateAuthAccessControl<Args extends { operation: keyof AuthAccess<Args> }>(
  args: { access: AuthAccess<Args> } & Args
) {
  // Either a boolean or an object describing a where clause
  let result: Static | Declarative = false;
  const acc = args.access[args.operation];
